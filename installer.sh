#!/bin/bash
## setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/Belfagor2005/revolutionxxx/main/installer.sh -O - | /bin/sh
exec > >(tee -a /tmp/revolutionx_debug.txt) 2>&1
set -x
...
TMPPATH=/tmp/revolutionxxx-main
...
wget --no-check-certificate 'https://github.com/Belfagor2005/revolutionxxx/archive/refs/heads/main.tar.gz'
tar -xzf main.tar.gz
cp -r 'revolutionxxx-main/usr' '/'
set +e
cd
sleep 2
...
IMAGE VERSION: $distro_version" >> /tmp/revolutionx_debug.txt
sleep 5
killall -9 enigma2
exit 0